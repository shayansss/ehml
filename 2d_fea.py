...
def set_element_type(modelObj, elemCode, instanceName='cartilage-1', elemLibrary=STANDARD):
    rootAssemblyObj = modelObj.rootAssembly
    rootAssemblyObj.setElementType(
        regions=(rootAssemblyObj.instances[instanceName].cells, ),
...
while True:
    try:
        ...
        surrogateParams['bcParams'] = {
            ...
            'u2': np.random.uniform(+0.0, 0.6),
            'u3': np.random.uniform(-0.5, 0.5),
            'ur1': np.random.uniform(-0.1,0.1),
            'ur2': np.random.uniform(-0.1,0.1),
            'ur3': np.random.uniform(-0.1,0.1),
            }
        surrogateParams['t'] = np.random.uniform(0.001,100)
        surrogateParams['amp'] = np.random.uniform(0, 1, 4)
        ...
        for key in ['lf', 'hf']:
            models[key].boundaryConditions['DISP'].setValuesInStep(
        ...
        # meshTools['lf'].record_k_nearest(pos['lf']['data'], pos['lf']['data'])
